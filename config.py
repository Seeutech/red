API_ID = 22741640  # api id
API_HASH = "c435fe7ad0a6398ccd9337839a3666e3"  # api hash

BOT_TOKEN = "6691256414:AAEGvXlLtVMCuyjkOJs18IWSJmgUcRNSeIM"  # bot token


## REDIS
HOST = "redis-15392.c304.europe-west1-2.gce.cloud.redislabs.com"  # redis host uri
PORT = "15392"  # redis port
PASSWORD = "cbw7xHuW3uJmddWJRRqxiSWmRvKi81GM"  # redis password

PRIVATE_CHAT_ID = -1002124678832  # CHAT WHERE YOU WANT TO STORE VIDEOS
COOKIE = "csrfToken=BnQx-ffuJKamkWLrsx3Ksds4; browserid=qEGrcJcpudjTVSp34yywiOK8KGczvjGYTPI6jJMydd7dpoQVUXCzDlp7JFM=; lang=en; __bid_n=18db31bdad1e0ee8094207; __stripe_mid=a300821a-4b38-4e0a-a227-5c27f1b7c54789f06e; __stripe_sid=f7b244b5-18a8-4540-bfa0-222b4acc0bf5eb56ba; ndus=Yb47bKTteHuiwDuAJYscQHtlthgbg2-2R4yRRWFe; ndut_fmt=17DAF2FD0DC6DF723D10EB645D49FD8581EFDCCB4A2823D11359EA0CCE270FBE"  # COOKIE FOR AUTHENTICATION (get from chrome dev tools) ex: "PANWEB=1; csrfToken=; lang=en; TSID=; __bid_n=; _ga=; __stripe_mid=; ndus=; browserid==; ndut_fmt=; _ga_06ZNKL8C2E=" (dont use this)
ADMINS = [6271019610]
PREMIUM_USERS = [6271019610, 5952619139]
YOUR_ADMIN_USERNAME = "xTMAdminBot"
PREMIUM_PLANS = {
    "𝐏𝐥𝐚𝐧 1": {"amount": 30.0, "validity_days": 15},
    "𝐏𝐥𝐚𝐧 2": {"amount": 60.0, "validity_days": 30},
    "𝐏𝐥𝐚𝐧 3": {"amount": 90.0, "validity_days": 45},
}
